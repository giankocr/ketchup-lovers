...
function agregar_boton_canjear_prenda_checkout() {
    ...
    if ( ! is_user_logged_in() ) {
        ...
        echo '<div class="canjear-prenda-container">
            <a href="/login" class="boton-estandar login-button">INICIAR SESIÓN PARA CANJEAR</a>
        </div>';
    } else {
...
<a href="#" class="boton-estandar canjear-prenda-button" style="display:none;" data-checkout-url="' . esc_url( $url_checkout ) . '">CANJEAR PRENDA AHORA</a>
...
function cargar_script_canjear_prenda() {
    ...
    if ( is_product() ) {
        wp_enqueue_script(
            ...
            get_stylesheet_directory_uri() . '/assets/js/canjear-prenda.js', // Ruta al archivo JS (lo crearemos en el siguiente paso)
            array( 'jquery' ), // Dependencia: necesita jQuery para funcionar
            '1.0.1', // Versión
